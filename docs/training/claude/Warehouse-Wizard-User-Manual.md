...
**Owner:** Simplex Trading Inc  
**Approved by:** Russell Hunte  
**Support / escalation:** russelljhunte@gmail.com 12462439703
...
> **Draft addendum notice:** Sections 1–25 below are the unedited WW-UM-001 content. Section 26 is a new addendum drafted during a training-materials review to cover topics present in `src/lib/help-content.ts` and `docs/admin-guide.md` but not yet in this manual (NetSuite integration, expanded label/printing standards, Warehouse Brain, badge sign-in detail, RF disconnect acknowledgment, and glossary additions). It has not been through owner/approver sign-off — treat it as a proposal for the next revision, not controlled floor documentation. See `docs/training/claude/Improvement-Notes.md` for the full review.
...
## 26. Addendum — extended technical reference (draft, unapproved)

This section was drafted from `src/lib/help-content.ts` and `docs/admin-guide.md` to close gaps found during a training-materials review. It follows the numbering of the sections above it but has not been through the owner/approver sign-off described in the front matter.

### 26.1 NetSuite integration overview {#help-netsuite-integration}

**Parallel Help ID:** `netsuite-integration`  
**Normal owners:** Admins and IT.

NetSuite is the first-class ERP adapter. It syncs items, purchase orders, item receipts, sales orders, transfer orders, fulfillments, and inventory adjustments between Warehouse Wizard and NetSuite.

- Configure NetSuite as the first integration connection during setup, and map items, locations, orders, receipts, fulfillments, and inventory adjustments before relying on synced data operationally.
- Every sync job uses an idempotency key and preserves the raw payload log, so a retried or duplicated webhook does not create a duplicate product, order, receipt, or adjustment.
- Use external record links (the NetSuite internal ID stored against the local record) to confirm which side is authoritative before correcting a mismatch.
- Repeated sync failures move into a dead-letter state with a reason and a resolution owner. Treat a dead-lettered sync the same as any other System Log exception: verify the underlying record, assign an owner, and do not mark it resolved until the sync succeeds or is intentionally excluded.
- This is an admin/IT configuration concern, not a floor workflow. Operators, clerks, and supervisors should never need to think about NetSuite directly — if sync state is visibly affecting floor work (a receipt not appearing, a fulfillment stuck), escalate to admin/IT rather than working around it.

### 26.2 Zebra/ZPL printing and label standards (expanded) {#help-zebra-printing}

**Parallel Help ID:** `zebra-printing`  
**Extends:** Section 15, "Labels and printing."

- Enterprise printing is ZPL-first. Printer stations, label templates, print jobs, and print failures are all tracked so any label can be reprinted with a full audit trail — this is why Section 15 says not to create a new record just to replace a damaged label.
- Browser printing is the approved office fallback only. It should not be the primary path for warehouse floor stations.
- Warehouse labels (pallet, location, carton, pick list, transfer, count sheet) use Code 128-compatible barcodes. Customer-facing logistics labels commonly need GS1-128, which carries an SSCC (Serial Shipping Container Code) rather than an internal identifier.
- Keep barcode values short, unique, and aligned with NetSuite item/location identifiers where the integration is active — a label that carries every possible data point scans slower and fails more often than one built for speed and reliability first.

### 26.3 Warehouse Brain recommendations (expanded) {#help-warehouse-brain}

**Parallel Help ID:** `warehouse-brain`  
**Extends:** Section 5.1 (Office view) and Section 24.1 (Dashboard).

Warehouse Brain is the recommendation engine behind the Office dashboard's "management recommendations." It continuously reviews live inventory, open work, holds, quarantine, expiration dates, cycle-count variance, dock status, and role context, and surfaces DPMO (defects per million opportunities) alongside fill level, turn, and expiration signals.

- Every recommendation must state its reason and its next action — it is a decision aid, not an automated action. A human still accepts, dismisses, or resolves it.
- Review recommendations at the same three points as the rest of the Office dashboard: shift start, before releasing a major wave, and end of shift (Section 5.2).
- Accept, dismiss, or resolve with a reason code once the recommendation has been checked against the source module or Inventory Search — never act on the recommendation text alone, for the same reason Section 5.1 says dashboard numbers are signals, not decisions.

### 26.4 Trusted badge sign-in — full rule (expanded) {#help-user-management-badge}

**Extends:** Section 3.1 and Section 18.

- Badge/PIN sign-in is a shortcut available only after a normal password sign-in has established trust on that specific mobile or tablet device.
- It is disabled on desktop entirely — desktop always requires the normal password sign-in.
- Trust expires after 30 days. After expiry, the device falls back to a normal password sign-in, which re-establishes trust for another 30 days.
- The server checks the active user, badge/PIN hash, trusted device ID, active trust record, expiry, client IP, and non-desktop client before creating a session from a badge. A failed check falls back to normal sign-in without revealing which specific check failed — do not try to diagnose which check failed from the floor; if badge sign-in stops working, re-authenticate with a password and tell a supervisor if it keeps happening.

### 26.5 Critical RF disconnect acknowledgment — full rule (expanded) {#help-system-log-operations}

**Parallel Help ID:** `system-log-operations`  
**Extends:** Section 16.2, Section 17, and Section 5's daily review.

- When a floor device drops offline, the system writes a critical RF disconnect entry to the System Log and shows a red acknowledgment toast to supervisors, managers, admins, and developers — not to the operator on the disconnected device.
- The toast requires a typed challenge (not just a click) before it can be dismissed. This is intentional friction: it exists so a critical disconnect cannot be cleared reflexively.
- Only type the acknowledgment after the operator has reconnected, the module has refreshed live state, and the floor work has been verified safe to continue (Section 16.3). Acknowledging the toast before that sequence is complete defeats its purpose.

### 26.6 Additional glossary terms

| Term | Meaning |
|---|---|
| DPMO | Defects per million opportunities; a Warehouse Brain/Office-dashboard quality signal. |
| ERP | Enterprise resource planning system; NetSuite is the first-class ERP adapter for this product. |
| GS1-128 | A GS1 barcode standard commonly required for customer-facing logistics labels; carries structured data such as an SSCC. |
| SSCC | Serial Shipping Container Code; a GS1-128 identifier used on customer-facing logistics labels. |
