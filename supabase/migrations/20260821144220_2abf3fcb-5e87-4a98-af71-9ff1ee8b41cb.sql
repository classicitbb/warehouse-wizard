...
SELECT r.id, f.id,
  ...
  CASE
    WHEN r.code IN ('admin', 'developer') THEN true
    WHEN r.code = 'warehouse_manager' AND f.code IN ('receiving','putaway','inventory','pick_lists','location_moves','cycle_counts','transfers','products','reports','status') THEN true
