ALTER TABLE public.operator_tickets
...
DROP POLICY IF EXISTS "Reporters and support read ticket screenshots" ON storage.objects;
CREATE POLICY "Reporters and support read ticket screenshots"
...
DROP POLICY IF EXISTS "Support deletes ticket screenshots" ON storage.objects;
CREATE POLICY "Support deletes ticket screenshots"
...
begin
  ...
  if coalesce(new.screenshot_path, '') <> '' then
    shot_note_html := '<p style="margin:0 0 16px;color:#334155;font-size:14px;line-height:1.5;">'
      || 'A screenshot of the screen was captured when this report was started. '
      || 'Open it in <a href="https://warehousewizard.app/settings?tab=support-requests" '
      || 'style="color:#0f766e;">Settings &rarr; Support Requests</a>.</p>';
