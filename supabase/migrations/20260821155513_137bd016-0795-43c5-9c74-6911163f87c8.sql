...
DROP TRIGGER IF EXISTS operator_tickets_notify_submitted ON public.operator_tickets;
CREATE TRIGGER operator_tickets_notify_submitted
AFTER UPDATE OF status ON public.operator_tickets
FOR EACH ROW
WHEN (new.status = 'open' AND old.status IS DISTINCT FROM 'open')
EXECUTE FUNCTION public.notify_operator_ticket_submitted();

DROP TRIGGER IF EXISTS operator_tickets_notify_inserted ON public.operator_tickets;
CREATE TRIGGER operator_tickets_notify_inserted
AFTER INSERT ON public.operator_tickets
FOR EACH ROW
WHEN (new.status = 'open')
EXECUTE FUNCTION public.notify_operator_ticket_submitted();

REVOKE EXECUTE ON FUNCTION public.notify_operator_ticket_submitted() FROM PUBLIC, anon, authenticated;