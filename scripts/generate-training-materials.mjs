...
const manualHtml = `<!doctype html><html><head><meta charset="utf-8"><style>${manualCss}</style></head><body><div class="cover"><div class="eyebrow">WAREHOUSE WIZARD</div><h1>User Manual</h1><p class="meta">End-to-end operating guide • Product 1.27 • WW-UM-001 • Revision 1.1</p><p>Managers • Supervisors • Clerks • Operators • Dispatch • Admins</p></div>${manualBody}</body></html>`;
...
const doc = new Document({
  ...
  sections: [{
    ...
    footers: { default: new Footer({ children: [new Paragraph({ alignment: AlignmentType.RIGHT, children: [new TextRun({ text: "WW-UM-001  •  Revision 1.1  •  Page ", color: C.gray, size: 16 }), new TextRun({ children: [PageNumber.CURRENT], color: C.gray, size: 16 })] })] }) },
    children: [
      ...
      new Paragraph({ children: [new TextRun({ text: "Product 1.27  •  WW-UM-001  •  Revision 1.1  •  Effective 17 August 2026", bold: true, color: C.orange })] }),
...
const manualIndex = headingMatches.map((m, i) => {
  ...
  return { id, title: m[2], productVersion: "1.27", revision: "1.1", source: "WW-UM-001", content: manualMd.slice(start, end).trim() };
});
fs.writeFileSync(path.join(outDir, "Warehouse-Wizard-User-Manual-AI-Index.json"), JSON.stringify({ documentId: "WW-UM-001", productVersion: "1.27", revision: "1.1", chunks: manualIndex }, null, 2));
...
const currentQuickHtml = quickHtml
  .replace("MOVE / TRANSFER / COUNT", "MOVE / COUNT / CORRECT")
  .replace("<li>Different warehouse = Transfer + driver sign-off</li>", "<li>Cross-warehouse Transfers are temporarily disabled</li>")
  .replace("<li>Use Inventory Search as the truth check</li>", "<li>Correct eligible pallets from Inventory Detail</li><li>Use Inventory Search as the truth check</li>");
fs.writeFileSync(quickHtmlPath, currentQuickHtml);
