...
&nbsp;", with the button reading "Update pack standard". Selecting a
...
4. **Optional build detail** — carton dimensions, deck/slip heights, footprint,
  tare, tolerance, notes, collapsed by default with a summary bar showing any values.
...
- Migration: `alter table permission_features add column is_released boolean not null default false` (additive) plus an insert of the `pack_designer` feature
