-- Web push notification spine (previously unapplied) plus scoped recipients.
...
-- Recipients for one notification: only devices whose owner may see the
-- warehouse the work belongs to, and who has not switched that alert off.
create or replace function public.notification_push_recipients(
  in_kind text,
...
set search_path = public
as $$
  select s.id, s.endpoint, s.p256dh, s.auth, s.failure_count
    from public.push_subscriptions s
    join public.profiles pr on pr.id = s.user_id
    left join public.user_notification_preferences np on np.user_id = s.user_id
   where coalesce(pr.approved, true)
     and coalesce(pr.active, true)
     and (in_kind <> 'pick_list_created' or coalesce(np.pick_list_ring, true))
     and (in_kind <> 'putaway_task_created' or coalesce(np.putaway_badge, true))
     and (
       in_warehouse_id is null
       or exists (
         select 1
           from public.user_roles ur
          where ur.user_id = s.user_id
            and ur.is_hidden = false
            and (ur.warehouse_id is null or ur.warehouse_id = in_warehouse_id)
       )
     );
$$;
...
revoke execute on function public.notification_push_recipients(text, uuid) from public, anon, authenticated;
...
grant execute on function public.notification_push_recipients(text, uuid) to service_role;
